# compass — next-best-prompts

A pointer to your best next moves. **compass** is a tiny MCP server that carries
...
The rule lives in [`rule/compass.md`](rule/compass.md) (single source of truth)
and is code-generated into a shared module that both transports import:

- **stdio** (`src/`) — published to npm as `@danzimon/compass-mcp`. For local
...
Both expose identical surfaces: the `instructions` field, a `compass` prompt, and
...
```bash
claude mcp add compass -- npx -y @danzimon/compass-mcp
```
...
hook in [`hook/sessionstart-compass.sh`](hook/sessionstart-compass.sh).)
...
{ "mcpServers": { "compass": { "command": "npx", "args": ["-y", "@danzimon/compass-mcp"] } } }
```
...
```
https://compass-mcp-remote.vercel.app/api/mcp
```
...
compass is available as an on-demand prompt/tool — for automatic every-turn
...
```bash
npm install
npm run embed        # rule/compass.md + shared/compass.template.ts -> generated modules
npm run check-sync   # fail if the generated copies drifted from source
npm run build        # tsc -> dist/ (prebuild runs embed)
```

Edit the rule in `rule/compass.md` or the prompt/tool wiring in
`shared/compass.template.ts`, then `npm run embed`. `check-sync` (run on
